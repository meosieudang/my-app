...
export class CoreWheel {
    constructor(options, drawWheel) {
        ...
        let defaultOptions = {
            ...
            'wheelImage': null,
            'pin': null,
            'imageDirection': 'N', //- (N)orth, (E)ast, (S)outh, (W)est.
            'drawButton': null,
            'drawBG': null,
        };
        ...
        if ((options != null) && (options['pin']) && (typeof (options['pin']) !== 'undefined')) {
            this.pin = new Pin(options['pin']);
        }
        ...
        if (drawWheel == true) {
            ...
            for (let y = 1; y <= this.numSegments; y++) {
                if (this.segments[y].image !== null && this.segments[y].imgData === null) {
    ...
    draw(clearTheCanvas) {
        if (this.ctx) {
            ...
            this.drawBG?.(this.canvas, this.ctx);
            ...
            this.drawButton?.(this.canvas, this.ctx);
            if (typeof this.pin !== 'null') {
                this.drawPin();
            }
    ...
    drawPin() {
        if (this.pin) {
            let imgData = this.pin.imgData
            let w = imgData.width;
            let h = imgData.height;
            let x = this.pin.x, y = this.pin.y;
            this.ctx.save();
            this.ctx.translate(x, y);
            this.ctx.drawImage(this.pin.imgData, - w / 2, - h / 2, w, h);
            this.ctx.restore();
        }
    }
        ...
        if (this.ctx && this.segments) {
            for (let x = 1; x <= this.numSegments; x++) {
                ...
                if (seg.imgData.height) {
                    ...
                    this.ctx.drawImage(seg.imgData, imageLeft, imageTop + seg.imageSpaceTop);
                    this.ctx.restore();
                }
                else {
    ...
    computeAnimation() {
        if (this.animation) {
            ...
            if (this.animation.stopAngle == null) {
                ...
                this.animation._stopAngle = (360 - this.animation.stopAngle + this.pointerAngle) % 360;
            }
...
function winwheelStopAnimation(canCallback) {
    if (canCallback != false) {
        winwheelToDrawDuringAnimation.animation.callbackFinished?.(winwheelToDrawDuringAnimation.getIndicatedSegment());
    }
}
